...
export const ProductInputSchema = z.object({
    ...
    countInStock: z.coerce.number().int().nonnegative('Stock of reviews must be a non-negative number'),
    tags: z.array(z.string()).default([]),
    sizes: z.array(z.string()).default([]),
    colors: z.array(z.string()).default([]),
    avgRating: z.coerce.number().min(0, 'Average rating must be at least 0 ').max(5, 'Average rating must be at least 5'),
    ...
    numSales: z.coerce.number().int().nonnegative('Stock of reviews must be a non-negative number'),
